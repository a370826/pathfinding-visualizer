    ...
    while True:
        ...
        for i in range(-1,2):
            for j in range(-1,2):
                ...
                try:
                    if (current[0]+i,current[1]+j) in visit: 
                        continue

                    elif (current[0]+i,current[1]+j) in reach:
                        changeParent(current[0]+i,current[1]+j)
...
def changeParent(x,y):
    ...
    grid[x][y].setCost()
    reach_cost[reach.index((lst[a][0],lst[a][1]))] = grid[x][y].cost()
...
def drawPath():
    ...
    while True:
        ...
        if (grid[current[0]][current[1]].x,grid[current[0]][current[1]].y) == startpos:
            break
